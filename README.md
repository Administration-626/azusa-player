...
由于 B 站的 CDN 调度策略，部分高质量音视频会被分配到 **MCDN (Peer-to-Peer CDN)** 节点：
- **现象**：这类链接在浏览器扩展环境、开启代理工具（如 Clash）或者特定宽带网络下极不稳定，经常返回 **404 Not Found** 导致播放中断。
- **插件优化**：Azusa-Player 已内置智能筛选逻辑，会自动识别并避开 MCDN 节点，优先尝试连接官方标准的 CDN 镜像。
- **建议**：如遇到某首歌无法播放，请尝试重新切歌或刷新插件，程序会自动搜索其他可用的备份线路。
